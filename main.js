function startSpaceFight(mode, playerName) {
  ...
  function drawBullets() {
    player.bullets.forEach(b => {
      b.trail.forEach((t, i) => { ctx.globalAlpha = i / 5; ctx.fillStyle = "yellow"; ctx.beginPath(); ctx.arc(t.x - camera.x, t.y - camera.y, 5 * (i / 5 + 0.2), 0, Math.PI * 2); ctx.fill(); });
      ctx.globalAlpha = 1; ctx.fillStyle = "yellow"; ctx.beginPath(); ctx.arc(b.x - camera.x, b.y - camera.y, 5, 0, Math.PI * 2); ctx.fill();
    });
    bulletsEnemy.forEach(b => {
      b.trail.forEach((t, i) => { ctx.globalAlpha = i / 8; ctx.fillStyle = "orange"; ctx.beginPath(); ctx.arc(t.x - camera.x, t.y - camera.y, 5 * (i / 8 + 0.2), 0, Math.PI * 2); ctx.fill(); });
      ctx.globalAlpha = 1; ctx.fillStyle = "orange"; ctx.beginPath(); ctx.arc(b.x - camera.x, b.y - camera.y, 5, 0, Math.PI * 2); ctx.fill();
    });
  }
  ...
  if (!isMobile) {
    ...
    stick.addEventListener("touchend", e => { dragging = false; joystick.dx = 0; joystick.dy = 0; stick.style.transform = "translate(0px,0px)"; });
